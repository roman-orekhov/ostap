...
class ['b] lexer s =
   ...
   let infixop3 = Str.regexp (sprintf "mod\|land\|lor\|lxor\|[*/%%]%s*" symbolchar) in
   let infixop4 = Str.regexp (sprintf "lsl\|lsr\|asr\|\*\*%s*" symbolchar) in
   ...
   let string = Str.regexp (sprintf "\"\([^\\\"]\|%s\)*\"" escaped) in
